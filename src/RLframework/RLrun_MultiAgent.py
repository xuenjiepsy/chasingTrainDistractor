...
class RunTimeStep:
    def __init__(self, actOneStepForWolfAndSheep, actOneStepForMaster, actOneStepForDistractor, sampleOneStep, learnFromBuffer, observeWolfAndSheep = None, observeMaster = None, observe = None):
        self.actOneStepForWolfAndSheep = actOneStepForWolfAndSheep
        self.actOneStepForMaster = actOneStepForMaster
        self.actOneStepForDistractor = actOneStepForDistractor
        self.sampleOneStep = sampleOneStep
        self.learnFromBuffer = learnFromBuffer
        self.observeWolfAndSheep = observeWolfAndSheep
        self.observeMaster = observeMaster
        self.observe = observe

    def __call__(self, state, replayBuffer, trajectory):
        runTime = len(trajectory)
        observationWolfAndSheep = self.observeWolfAndSheep(state) if self.observeWolfAndSheep is not None else state
        observationMaster = self.observeMaster(state) if self.observeMaster is not None else state
        observation = self.observe(state) if self.observe is not None else state
        actionWolfAndSheep = self.actOneStepForWolfAndSheep(observationWolfAndSheep, runTime)
        actionMaster = self.actOneStepForMaster(observationMaster, state)
        actionDistractor = self.actOneStepForDistractor(observation, runTime)
        action = actionWolfAndSheep + actionMaster + actionDistractor
