...
class ActOneStepRecreate:
    def __init__(self, actByTrainNoisy, masterID, wolvesID, sheepsID, getPosFromState, forceDistanceForSheep, force, forceSigma):
        self.actByTrain = actByTrainNoisy
        ...
        self.force = force
        self.forceSigma = forceSigma
        # self.masterRealPullDistanceWM = masterRealPullDistanceWM


    def __call__(self, model, allAgentsStatesBatch, state):
        allAgentsStates = np.array(allAgentsStatesBatch)[None]
        for wolvesID in self.wolvesID:
            ...
            for sheepsID in self.sheepsID:
                ...
                distanceX = sheepsPos[0] - wolvesPos[0]
                distanceY = sheepsPos[1] - wolvesPos[1]
                distance = math.sqrt(distanceX**2 + distanceY**2)
                for masterID in self.masterID:
                    masterState = state[masterID]
                    masterPos = self.getPosFromState(masterState)
                    gradient = ((masterPos[1]-wolvesPos[1])/(masterPos[0]-wolvesPos[0]))
                    dirFactor = -1 if gradient * (masterPos[0]-wolvesPos[0]) > 0 else 1
                    dirFactor2 = -1 if gradient > 0 else 1
                    angle = math.atan(gradient)
                    force = np.random.normal(self.force, self.forceSigma, size=None)
                    # distanceWMx =  masterPos[0] - wolvesPos[0]
                    # distanceWMy =  masterPos[1] - wolvesPos[1]
                    # distanceWM = math.sqrt(distanceWMx**2 + distanceWMy**2)
                    if distance < self.forceDistanceForSheep:
                        actionX = force * math.cos(angle) * dirFactor * dirFactor2
                        actionY = force * math.sin(angle) * dirFactor * dirFactor2
                        actions = [0, actionX, 0, actionY, 0]
                    # elif distanceWM < self.masterRealPullDistanceWM:
                    #     actionX = self.force * math.cos(angle) * dirFactor * dirFactor2*0.5
                    #     actionY = self.force * math.sin(angle) * dirFactor * dirFactor2*0.5
                    #     actions = [0, actionX, 0, actionY, 0]
                    else:
                        actions = [0, 0, 0, 0, 0]
        return actions
