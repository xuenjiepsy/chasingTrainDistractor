...
class DrawSingleArrow():
    ...
    def __call__(self,ax,sheepPos,action):
        ...
        # print(action)
        ax.arrow(sheepPos[0], sheepPos[1], action[0], action[1], width=0.01,length_includes_head=True,head_width=0,head_length=0,fc='green',ec='green')
...
def makeDirectionMap(condition):

    damping = condition['damping']
    frictionloss = condition['frictionloss']
    masterForce = condition['masterForce']
    # offset = condition['']
    killZoneRatio = condition['killZone']#4.0
    ropePunishWeight = condition['ropePunishWeight']#0.1 0.3 0.5
    ropeLength = condition['ropeLength'] #
    masterMass = condition['masterMass']
    ...
    # ropePunishWeight = 0.1#0.1 0.3 0.5
    # ropeLength = 0.06 #
    # masterMass = 1.0
    # dt = 0.02
    ...
    for yPos in posValue[rowName]:
        for xPos in posValue[columnName]:
            ...
            plotCounter += 1
            print(plotCounter)
    plt.suptitle('chasingMap_damping={}_frictionloss={}_killZoneRatio{}_masterForce={}_masterMass={}_ropeLength={}_ropePunishWeight={}'.format(damping,frictionloss,killZoneRatio,masterForce,masterMass,ropeLength,ropePunishWeight))

def main():

    manipulatedVariables = OrderedDict()
    manipulatedVariables['damping'] = [0.0,0.5]
    manipulatedVariables['frictionloss'] = [0.0,1.0]
    manipulatedVariables['masterForce'] = [0.0,1.0]
    manipulatedVariables['killZone'] = [3.0]
    manipulatedVariables['ropePunishWeight'] = [0.1]#()
