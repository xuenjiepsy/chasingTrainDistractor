...
def generateSingleCondition(condition):
    ...
    evalNum = 4
    maxRunningStepsToSample = 500
    modelSaveName = 'trainSeperated'
    ...
    modelFolderForWolfAndSheep = os.path.join(mainModelFolder, 'trainWolfAndSheep',
                                              'sheepForce={},wolfForce={}'.format(sheepForce, wolfForce))
    ...
    modelFolderMaster = os.path.join(mainModelFolder, 'trainMasterChangeName',
                                              'masterPullForce={},masterForce={}'.format(masterPullForce,masterForce))
    ...
    sampleTrajectory = SampleExpTrajectoryWithAllFrames(maxRunningStepsToSample, transit, isTerminal, reset)
    ...
    [restoreVariables(model, path) for model, path in zip(modelsListForMaster[3:5], modelPathsForDistractor)]
    modelList = modelsListForWolfAndSheep + [modelsListForMaster[2]] + modelsListForMaster[3:5]
    ...
    actOneStepForMaster = lambda allAgentsStates: [actOneStepOneModel(model, allAgentsStates) for model in modelsListForMaster]
    ...
    for hideId in hideIdList:
        ...
        if saveTraj:
            ...
            trajectoriesSaveDirectory= os.path.join(dataFolder,'trajectorySeperated',modelSaveName,'ENDnoiseOffsetWithMasterPull')
            if not os.path.exists(trajectoriesSaveDirectory):
                os.makedirs(trajectoriesSaveDirectory)

            trajectorySaveExtension = '_forexp.pickle'
            fixedParameters = {'evalNum':evalNum,'hideId':hideId}
            generateTrajectorySavePath = GetSavePath(trajectoriesSaveDirectory, trajectorySaveExtension, fixedParameters)
            trajectorySavePath = generateTrajectorySavePath(condition)
            saveToPickle(trajList, trajectorySavePath)

            expTrajectoriesSaveDirectory = os.path.join(dataFolder, 'ExptrajectorySeperated', modelSaveName,'ENDnoiseOffsetWithMasterPull')
        ...
        if visualizeTraj:
            ...
            pictureFolder = os.path.join(dataFolder, 'demo', modelSaveName,'trainSeperated','ropeLength={},masterForce={}'.format(ropeLength,masterForce))
...
def main():
 ...
 #ssr-1, ssr-3 = 1.0; Xp = 2.0
    ...
    manipulatedVariables['wolfForce'] = [6.0]
