...
from src.functionTools.loadSaveModel import saveToPickle, restoreVariables,GetSavePath, ScaleTrajectory
...
def generateSingleCondition(condition):
    ...
    modelSaveName = 'trainDistractorKillZoneExpDemo'
    ...
    modelFolderForDistractor = os.path.join(mainModelFolder, 'trainDistractorKillZone','distractorForce={},distractKillZoneRatio={}'.format(distractorForce,distractKillZoneRatio))
    ...
    masterID = [2]
    distractorID = [3, 4, 5]
    hideIdList = hideId# distractorID + sheepsID
    ...
    distractorReshapeAction=ReshapeAction(distractorForce)
    ...
    if noiseDistractor:
          reshapeActionList = [ReshapeAction(wolfForce),ReshapeAction(sheepForce),ReshapeAction(masterRealForce),noiseDistractorAction,noiseDistractorAction,noiseDistractorAction]
    else:
    ...
    [restoreVariables(model, path) for model, path in zip(modelsListForDistractor[3:6], modelPathsForDistractor)]
    ...
    actOneStepForDistractor = lambda allAgentsStates: [actOneStepOneModel(model, allAgentsStates) for model in modelsListForDistractor[3:6]]
    ...
    for hideId in hideIdList:
        ...
        for hideIdeach in hideId:
            del(agentList[hideIdeach])
        hideId.reverse()
        trajList = []
        expTrajList = []
        newTrajList = []
        for _ in range(evalNum):
            # np.random.seed(i)
            traj, expTraj = sampleTrajectory(actOneStepForWolfAndSheep, actOneStepForMaster, actOneStepForDistractor, observeWolfAndSheep, observeMaster, observe)
        ...
        if saveTraj:
            ...
            trajectoriesSaveDirectory= os.path.join(dataFolder,'trajectorySeperatedNoDistactor',modelSaveName,'ENDnoiseOffset50Rdemo')
            if not os.path.exists(trajectoriesSaveDirectory):
                os.makedirs(trajectoriesSaveDirectory)

            trajectorySaveExtension = '_forexp40R.pickle'
            fixedParameters = {'hideId0':hideId[0],'hideId1':hideId[1]}
            generateTrajectorySavePath = GetSavePath(trajectoriesSaveDirectory, trajectorySaveExtension, fixedParameters)
            trajectorySavePath = generateTrajectorySavePath(condition)
            saveToPickle(trajList, trajectorySavePath)

            expTrajectoriesSaveDirectory = os.path.join(dataFolder, 'ExptrajectorySeperated', modelSaveName,'ENDnoiseOffset40RExp')
        ...
        if visualizeTraj:
            ...
            pictureFolder = os.path.join(dataFolder, 'demo', modelSaveName,'V4ChasingNoConsdistractorForce={},distractKillZoneRatio={}'.format(distractorForce,distractKillZoneRatio))
...
def main():
 ...
 #ssr-1, ssr-3 = 1.0; Xp = 2.0
    ...
    manipulatedVariables['ropeLength'] = [0.05]
    ...
    manipulatedVariables['RealAllowedDistanceWM'] = [0.5]
