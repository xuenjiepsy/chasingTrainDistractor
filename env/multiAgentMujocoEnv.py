...
class ResetUniformWithoutXPosForWolfAndSheep:
...
class TransitionFunctionWithoutXPosNoVel:
    def __init__(self, simulation,numSimulationFrames,visualize, isTerminal, reshapeActionList):
        self.simulation = simulation
        self.isTerminal = isTerminal
        self.numSimulationFrames = numSimulationFrames
    ...
    def __call__(self, state, actions):
        ...
        for simulationFrame in range(self.numSimulationFrames):
            ...
            agentNewQPos = lambda agentIndex: newQPos[self.numJointEachSite * agentIndex : self.numJointEachSite * (agentIndex + 1)]
            # agentNewQVel = lambda agentIndex: newQVel[self.numJointEachSite * agentIndex : self.numJointEachSite * (agentIndex + 1)]
            agentNewState = lambda agentIndex: np.array([agentNewQPos(agentIndex)],[0,0])
            newState = np.asarray([agentNewState(agentIndex) for agentIndex in range(numAgent)])
...
class RewardWolf:
    def __init__(self, wolvesID, sheepsID, entitiesSizeList, getPosFromState, isCollision, BoundForWolfself, collisionReward=10):
        ...
        self.BoundForWolfself = BoundForWolfself
        self.getPosFromState = getPosFromState
    ...
    def __call__(self, state, action, nextState):
        ...
        for wolfID in self.wolvesID:
            ...
            for sheepID in self.sheepsID:
                ...
                wolvesState = state[wolfID]
                wolvesPos = self.getPosFromState(wolvesState)
                sheepsState = state[sheepID]
                sheepsPos = self.getPosFromState(sheepsState)
                distanceForsheepsX = sheepsPos[0] - wolvesPos[0]
                distanceForsheepsY = sheepsPos[1] - wolvesPos[1]
                wolfAction = action[wolfID]
                wolfReward -= self.BoundForWolfself(distanceForsheepsX, distanceForsheepsY, wolfAction[0], wolfAction[1])
        reward = [wolfReward] * len(self.wolvesID)
        # print('wolfreward ', wolfReward)
        return reward

class BoundForWolfself:
    def __init__(self, freeDistance, freeForce):
        self.freeDistance = freeDistance
        self.freeForce = freeForce

    def __call__(self, distanceSheepsX, distanceSheepsY, forceX, forceY):
        if (distanceSheepsX ** 2 + distanceSheepsY ** 2) < (self.freeDistance ** 2) and (forceX ** 2 + forceY ** 2) > (self.freeForce ** 2):
            return 8
        else:
            return 0
...
class RewardSheep:
    def __init__(self, wolvesID, sheepsID, entitiesSizeList, getPosFromState, isCollision, punishForOutOfBound,reshapeActionList, collisionPunishment=10):
        self.wolvesID = wolvesID
        self.getPosFromState = getPosFromState
        self.entitiesSizeList = entitiesSizeList
        self.sheepsID = sheepsID
    ...
    def __call__(self, state, action, nextState):  # state, action not used
        reward = []
        action = [reshapeAction(eachAction) for eachAction,reshapeAction in zip(action,self.reshapeActionList)]
        for sheepID in self.sheepsID:
            sheepReward = 0
            sheepNextState = nextState[sheepID]
...
class RewardMaster:
    def __init__(self, masterID, wolvesID, sheepsID, entitiesSizeList, getPosFromState, isCollision, punishForOutOfBound, BoundForWolves,collisionPunishment=0):
        self.getPosFromState = getPosFromState
        self.entitiesSizeList = entitiesSizeList
        ...
        self.collisionPunishment = collisionPunishment
        self.punishForOutOfBound = punishForOutOfBound
        self.BoundForWolves = BoundForWolves
        self.physicsDim = 2
    ...
    def __call__(self, state, action, nextState):  # state, action not used
        ...
        for masterID in self.masterID:
            ...
            for wolvesID in self.wolvesID:
                wolvesState = state[wolvesID]
                wolvesPos = self.getPosFromState(wolvesState)
                masterState = state[masterID]
                masterPos = self.getPosFromState(masterState)
                distanceForWolvesX = masterPos[0] - wolvesPos[0]
                distanceForWolvesY = masterPos[1] - wolvesPos[1]

                wolfSize = self.entitiesSizeList[wolvesID]
                masterSize = self.entitiesSizeList[masterID]
                wolfNextState = nextState[wolvesID]
                if self.isCollision(wolfNextState, masterNextState, wolfSize, masterSize):
                    masterReward -= self.collisionPunishment

                for sheepsID in self.sheepsID:
                    sheepsState = state[sheepsID]
                    sheepsPos = self.getPosFromState(sheepsState)
                    distanceForsheepsX = sheepsPos[0] - wolvesPos[0]
                    distanceForsheepsY = sheepsPos[1] - wolvesPos[1]
                    masterAction = action[masterID]
                    masterReward -= self.BoundForWolves(distanceForWolvesX, distanceForWolvesY, distanceForsheepsX, distanceForsheepsY, masterAction[0], masterAction[1])

                    sheepSize = self.entitiesSizeList[sheepsID]
                    masterSize = self.entitiesSizeList[masterID]
                    sheepNextState = nextState[sheepsID]
                    if self.isCollision(sheepNextState, masterNextState, sheepSize, masterSize):
                        masterReward -= self.collisionPunishment
...
class BoundForWolves:
    def __init__(self, forceDistance,forceDistanceForSheep, distancePunish, pullForce):
        self.forceDistance = forceDistance
        self.forceDistanceForSheep = forceDistanceForSheep
        self.distancePunish = distancePunish
        self.pullForce = pullForce

    def __call__(self, distanceWolvesX, distanceWolvesY, distanceSheepsX, distanceSheepsY, forceX, forceY):
        if (distanceWolvesX ** 2 + distanceWolvesY ** 2) < (self.forceDistance ** 2) and (forceX ** 2 + forceY ** 2) > 0:
            return self.distancePunish * 5
        elif (distanceWolvesX ** 2 + distanceWolvesY ** 2) > (self.forceDistance ** 2) and (distanceSheepsX ** 2 + distanceSheepsY ** 2) > (self.forceDistanceForSheep ** 2):
            if (forceX ** 2 + forceY ** 2) > 0:
                return self.distancePunish * 5
            else:
                return 0
        elif (distanceWolvesX ** 2 + distanceWolvesY ** 2) > (self.forceDistance ** 2) and (distanceSheepsX ** 2 + distanceSheepsY ** 2) < (self.forceDistanceForSheep ** 2):
            if (forceX ** 2 + forceY ** 2) > (self.pullForce ** 2):
                return self.distancePunish * 16
            else:
                return 0
        else:
            return 0
